...
public class GetAccountCardHoldersUseCase {
    ...
    public Page<CardHolder> execute(UUID accountId, int page, int pageSize, String search, CardHolderStatus status,
            String sortBy, String sortDirection) {
        Pageable pageable = PageRequest.of(Math.max(0, page - 1), pageSize, buildSort(sortBy, sortDirection));
        return cardRepo.findAccountCardHoldersWithFilters(accountId, pageable, normalize(search), status);
    }
